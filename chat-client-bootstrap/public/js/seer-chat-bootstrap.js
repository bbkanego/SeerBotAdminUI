 ...
 *
 * Note that this JS will be loaded by the client on there website an thus we will use plain old JS here. No jquery etc
 * will be used here.
 */
window.SEER_CHAT_BOOTSTRAP = function (config) {
    ...
    const _destroy = function () {
        ...
        internalConf.iframeHandle.postMessage(internalConf.senderNonce + ':destroy:' + payloadString,
    ...
    const handleMessage = function (event) {
        ...
        switch (messageParts[0]) {
            ...
            case 'callback':
                ...
                if (externalConf.callbacks && externalConf.callbacks[callbackContext] &&
    ...
    const sendConfig = function (event, nonce) {
        ...
        event.source.postMessage(internalConf.senderNonce + ':setConfig:'
            + configString, internalConf.iframeTargetOrigin);
    }
